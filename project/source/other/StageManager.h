...
public:
	...
	const StageData& GetStageData(int32_t index) const {
		// 配列外参照を防ぐアサート
		assert(index >= 0 && index < static_cast<int32_t>(stageDates_.size()));
		return stageDates_[index];
	}

	// ステージ総数を取得
	uint32_t GetStageNum() const {
		return static_cast<uint32_t>(stageDates_.size());
	}
...
private:
	...
	std::vector<StageData> stageDates_;
};