...
void ObstacleFast::Initialize(Object3d* model, Camera* camera, const Vector3& position,Player* player)
{
    // nullポインタチェック
    assert(model);
    assert(camera);
...
void ObstacleFast::OnCollision(const Player* player)
{
    // プレイヤーの速度が一定以上なら破壊
    if (player->GetSpeedStage() >= SpeedStage::kFast)
    {
        if (isDead_) return;
